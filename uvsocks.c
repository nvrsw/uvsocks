  ...
  if (status < 0)
    {
      ...
      uvsocks_set_status (tunnel, UVSOCKS_ERROR_DNS_RESOLVED);
                   ...
                   int           status)
{
  ...
  if (status < 0)
    {
      uvsocks_set_status (tunnel, UVSOCKS_ERROR_TCP_CONNECTED);
      uvsocks_remove_session (tunnel, session);
      free (connect);
      return;
    }

  uvsocks_set_status (tunnel, UVSOCKS_OK_TCP_CONNECTED);
